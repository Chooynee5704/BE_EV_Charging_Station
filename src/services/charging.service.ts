...
import { ChargingSession, IChargingSession } from "../models/chargingsession.model";
...
export async function startCharging(input: StartChargingInput) {
  ...
  if (!Types.ObjectId.isValid(vehicleId)) throw Object.assign(new Error("vehicleId invalid"), { status: 400 });
  if (!Types.ObjectId.isValid(slotId)) throw Object.assign(new Error("slotId invalid"), { status: 400 });
  if (typeof initialPercent !== "number" || initialPercent < 0 || initialPercent > 100) {
    throw Object.assign(new Error("initialPercent must be 0..100"), { status: 400 });
  }
  if (targetPercent !== null && (targetPercent <= 0 || targetPercent > 100)) {
    throw Object.assign(new Error("targetPercent must be 1..100"), { status: 400 });
  }
  if (typeof chargeRatePercentPerMinute !== "number" || chargeRatePercentPerMinute <= 0) {
    throw Object.assign(new Error("chargeRatePercentPerMinute must be > 0"), { status: 400 });
  }
...
export async function stopCharging(sessionId: string, status: "completed" | "cancelled" = "completed") {
  if (!Types.ObjectId.isValid(sessionId)) throw Object.assign(new Error("sessionId invalid"), { status: 400 });
